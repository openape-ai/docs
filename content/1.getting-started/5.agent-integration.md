...
This guide walks through the complete agent lifecycle: enrollment, authentication, grant request, approval, and token usage. All examples use `curl` against a running IdP.
...
- An Ed25519 key pair for the agent

::callout{type="info"}
All examples use `https://id.example.com` as the IdP URL and `my-secret-management-token` as the Management Token. Replace these with your actual values.
::
